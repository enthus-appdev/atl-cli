...
func TestAddCommentWithOptionsPostsPrebuiltADF(t *testing.T) {
	prebuilt := BuildReplyADF(
		"example.atlassian.net",
		"PROJ-42",
		"987",
		&User{AccountID: "account-123", DisplayName: "Alex Example"},
		&ADF{Type: "doc", Version: 1, Content: []ADFContent{{Type: "paragraph", Content: []ADFContent{{Type: "text", Text: "The actual reply"}}}}},
	)
	...
	client := &Client{
		...
		httpClient: &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
			...
			if posted.Body == nil || len(posted.Body.Content) != 2 || len(posted.Body.Content[0].Content) != 3 {
			...
			link := posted.Body.Content[0].Content[2]
