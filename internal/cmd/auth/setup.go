...
func runSetup(opts *SetupOptions) error {
	...
	if opts.Interactive {
		...
		fmt.Fprintln(opts.IO.Out, "")
		fmt.Fprintln(opts.IO.Out, "    • Click "+output.Bold.Render("Add")+" next to \"Jira Software API\" (for sprints)")
		fmt.Fprintln(opts.IO.Out, "    • In the "+output.Bold.Render("Classic Scopes")+" tab, enable:")
		fmt.Fprintln(opts.IO.Out, "        "+output.Faint.Render("manage:jira-software"))
		fmt.Fprintln(opts.IO.Out, "")
